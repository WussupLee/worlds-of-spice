...
- Real Web Audio output is measured in an AudioWorklet after a flipper action, with music and wind set to zero, to catch an inaudible effects bus. This validates signal, not subjective speaker quality.
...
CPU-only CI uses one browser worker and action/DOM traces without continuous WebGL screencast readbacks. Explicit screenshots remain enabled. The full-game lifecycle advances the browser clock in quarter-second frames; collisions still advance at 240 Hz. Ordinary touch, pause, audio and layout tests use real elapsed time. AudioWorklet peak measurement avoids missing a short mechanical transient between slow graphics frames. The game itself bounds catch-up at 250 ms and automatically reduces raster density under sustained rendering pressure; no test-only physics or scripted drains are shipped.
